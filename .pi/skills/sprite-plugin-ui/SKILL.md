...
├── dock SpriteSplitterSide（ui/sprite_tool_side.tscn）    ← DOCK_SLOT_RIGHT_UL（右侧第一排）
│   └── VBox：TabContainer（切分/分析/去背景/导出，各页 ScrollContainer）/ 状态栏
└── dock SpriteSplitterEditDock（ui/sprite_tool_edit_dock.tscn） ← DOCK_SLOT_RIGHT_BL（第一排正下方，与 side 同列）
    └── 切片编辑面板（单选切片编辑名称/几何/锁定/忽略，多选占位提示）
```

- **双挂载 + 跨区域交互**：一个 EditorPlugin 可同时 `EditorInterface.get_editor_main_screen().add_child(main)` +
  `add_control_to_dock(DOCK_SLOT_RIGHT_UL, side)`；两视图 `set_controller(controller)` 绑定同一 SpsController，
...
- **输出根目录（统一 out 目录，父目录项目设置可配）**：ProjectSettings `sprite_tool/out_root`（默认 `res://out_sprites`，`_ensure_out_root_setting` 在 controller `_init` 注册 + `add_property_info` 使项目设置面板可见）；`get_out_root()/set_out_root(v, persist)`——**persist=true 且编辑器模式才 `ProjectSettings.save()` 写 project.godot**（headless 仅内存，测试注入不污染）；`default_out_dir(tag)` = `<root>/<tag>`（项目数据 out_dir 新建默认值，旧 `res://out_sprites/ui/<uid>` 作废）；**所有生成资源统一落根目录下**：新建 data 默认 out_dir、去背景 PNG（`_write_transparent_png`）、导出/去背景/sheet 空目录 fallback、Tools 快捷入口（原 `res://sprites`）；侧栏导出 tab「输出根目录」行（LineEdit + 默认按钮：`text_changed` 仅内存生效、`text_submitted`/`focus_exited`/默认按钮持久化，**全局设置不 mark_dirty**，`set_controller` 时 `_refresh_out_root()` 填充且不被 `data_loaded` 覆盖）。
